...
function setupEventListeners() {
  document.addEventListener("DOMContentLoaded", function () {
    ...
    const confirmStudentLogin = document.getElementById(
    ...
    if (cancelDriverLogin)
      cancelDriverLogin.addEventListener("click", () => {
    ...
    if (confirmDriverLogin)
      confirmDriverLogin.addEventListener("click", handleDriverLogin);
    if (confirmStudentLogin)
      confirmStudentLogin.addEventListener("click", handleStudentLogin);
    if (logoutBtn) logoutBtn.addEventListener("click", handleLogout);
    if (enableLocation)
      enableLocation.addEventListener("click", enableLocationHandler);
    ...
    if (startTracking)
      startTracking.addEventListener("click", startDriverTracking);
    if (stopTracking)
      stopTracking.addEventListener("click", stopDriverTracking);
...
async function handleDriverLogin() {
  ...
  const selectedBusOption = document.querySelector(
    "#driver-login-modal .bus-option.selected"
  );
...
async function handleStudentLogin() {
  ...
  const selectedBusOption = document.querySelector(
    "#student-login-modal .bus-option.selected"
  );
...
function trackBusHandler() {
  ...
  trackedBusMarker = L.marker([bus.lat, bus.lng], {
    ...
    .addTo(map)
    .bindPopup(`Tracked Bus: ${bus.name || selectedBusId}`)
    .openPopup();
...
function updateBusList(buses) {
  ...
  if (totalBuses === 0) {
    busList.innerHTML =
      '<div class="bus-item"><div class="bus-info"><h3>No buses available</h3></div></div>';
  }
}
...
function updateBusMarker(busId, lat, lng, status) {
  ...
  const marker = L.marker([lat, lng], {
    icon: L.divIcon({
      ...
      html: `<i class="fas fa-bus" style="color: ${
